...
#include "new_events.h"
		...
		namespace KingdomsWar
		{
			...
			std::string EVENT_STAT = "kw_stat", EVENT_MIN_LV = "kw_lv_min", EVENT_MAX_LV = "kw_lv_max", REWARD_VNUM = "kw_item_vnum", REWARD_COUNT = "kw_item_count",  REWARD_B_VNUM = "kw_t_item_vnum", REWARD_B_COUNT = "kw_t_item_count";
		...
		bool CMgrKingdomsWar::Start()
		{
			...
			{
				char buf[201] = {0};
				snprintf(buf, sizeof(buf), "%s",LC_TEXT(KingdomsWar::MSG[i]));//信息提示(1)
				BroadcastNotice(buf);
			}
		...
		bool CMgrKingdomsWar::Stop()
		{
			...
			{
				char buf[201] = {0};
				snprintf(buf, sizeof(buf), "%s",LC_TEXT(KingdomsWar::MSG[i]));//信息提示(3)
				BroadcastNotice(buf);
			}
		...
		bool CMgrKingdomsWar::Config(DWORD dwMinLv, DWORD dwMaxLv, DWORD r_dwItemVnum, DWORD r_dwItemCount, DWORD t_dwItemVnum, DWORD t_dwItemCount)
		{
			if (g_bChannel != KingdomsWar::MAP_CHANNEL)
				return false;
		...
		void CMgrKingdomsWar::DeclareWinner()
		{
			...
			if (bEmpire != 0)
			{
					...
					case 1:
						{
							...
							for (; iter != mPartecipants1.end(); ++iter)
							{
								...
								if ((reward.r_dwItemVnum > 0) && (reward.r_dwItemCount))
									DBManager::instance().Query("INSERT INTO item_award (pid, login, vnum, count, given_time, why, mall) VALUES(%d, '%s', %d, %d, NOW(), '%s', 1);", dwPlayerID, acc_name.c_str(), reward.r_dwItemVnum, reward.r_dwItemCount, "盛唐国奖励");
							...
							if (iterFind != mPartecipants1.end())
							{
								...
								if ((reward.b_dwItemVnum > 0) && (reward.b_dwItemCount))
									DBManager::instance().Query("INSERT INTO item_award (pid, login, vnum, count, given_time, why, mall) VALUES(%d, '%s', %d, %d, NOW(), '%s', 1);", dwTopPlayerID, acc_name.c_str(), reward.b_dwItemVnum, reward.b_dwItemCount, "盛唐国奖励 - 第一名");
							}
						}
						break;
					case 2:
						{
							...
							for (; iter != mPartecipants2.end(); ++iter)
							{
								...
								if ((reward.r_dwItemVnum > 0) && (reward.r_dwItemCount))
									DBManager::instance().Query("INSERT INTO item_award (pid, login, vnum, count, given_time, why, mall) VALUES(%d, '%s', %d, %d, NOW(), '%s', 1);", dwPlayerID, acc_name.c_str(), reward.r_dwItemVnum, reward.r_dwItemCount, "秦皇国奖励");
							...
							if (iterFind != mPartecipants2.end())
							{
								...
								if ((reward.b_dwItemVnum > 0) && (reward.b_dwItemCount))
									DBManager::instance().Query("INSERT INTO item_award (pid, login, vnum, count, given_time, why, mall) VALUES(%d, '%s', %d, %d, NOW(), '%s', 1);", dwTopPlayerID, acc_name.c_str(), reward.b_dwItemVnum, reward.b_dwItemCount, "秦皇国奖励 - 第一名");
							}
						}
						break;
					case 3:
						{
							...
							for (; iter != mPartecipants3.end(); ++iter)
							{
								...
								if ((reward.r_dwItemVnum > 0) && (reward.r_dwItemCount))
									DBManager::instance().Query("INSERT INTO item_award (pid, login, vnum, count, given_time, why, mall) VALUES(%d, '%s', %d, %d, NOW(), '%s', 1);", dwPlayerID, acc_name.c_str(), reward.r_dwItemVnum, reward.r_dwItemCount, "汉武国奖励");
							...
							if (iterFind != mPartecipants3.end())
							{
								...
								if ((reward.b_dwItemVnum > 0) && (reward.b_dwItemCount))
									DBManager::instance().Query("INSERT INTO item_award (pid, login, vnum, count, given_time, why, mall) VALUES(%d, '%s', %d, %d, NOW(), '%s', 1);", dwTopPlayerID, acc_name.c_str(), reward.b_dwItemVnum, reward.b_dwItemCount, "汉武国奖励 - 第一名");
							}
						}
						break;
				}
				...
				for (int i = 5; i < 8; ++i)
				{
					...
					if (i == 6)
						snprintf(buf, sizeof(buf), "%s",LC_TEXT(KingdomsWar::MSG[i]));//信息提示(7)
					else
			...
			else
			{
				...
				{
					char buf[201] = {0};
					snprintf(buf, sizeof(buf), "%s",LC_TEXT(KingdomsWar::MSG[i]));//信息提示(9)
					BroadcastNotice(buf);
				}
