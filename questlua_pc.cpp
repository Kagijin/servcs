...
namespace quest
{
	...
	ALUA(pc_destroy_guild)
	{
		...
		if (g)
		{
			...
			if (g->UnderAnyWar())
			{
				ch->ChatPacket (CHAT_TYPE_INFO, LC_TEXT ("Need to wait a few seconds"));
				return 0;
			}
			else
				// @fixme188 END
	...
	ALUA(pc_change_sex)
	{
		...
		if (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_BODY) || (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_HAIR)) || (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_WEAPON)))
		{
	...
	ALUA(pc_give_exp2)
	{
		...
		int64_t exp = (int64_t)rint(lua_tonumber(L, 1));
	...
	ALUA(pc_give_exp)
	{
		...
		int64_t exp = (int64_t)rint(lua_tonumber(L, 2));
	...
	ALUA(pc_give_exp_perc)
	{
		...
		int64_t exp = (int64_t)((exp_table[MINMAX(0, lev, PLAYER_MAX_LEVEL_CONST)] * proc) / 100);
		PC* pPC = CQuestManager::instance().GetCurrentPC();
		pPC->GiveExp(lua_tostring(L, 1), exp);
		return 0;
	}
	...
	ALUA(pc_select_vid)
	{
		...
		if (new_ch)
		{
			...
			// lua_pushnumber(L, (DWORD)ch->GetVID());//ÎÒ×¢ÏúµÄ
		...
		else
		{
			...
			if (!tch)
			{
				...
				if (pkCCI)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "Ä¿±êÎ»ÓÚ%dÆµµÀ£¨ÎÒµÄÆµµÀ%d£©", pkCCI->bChannel, g_bChannel);
					}
					else
					{
						PIXEL_POSITION pos;
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "ÕÒ²»µ½µØÍ¼(Ë÷Òý%d)", pkCCI->lMapIndex);
						}
						else
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "ÄãÒÆ¶¯µ½( %d, %d )", pos.x, pos.y);
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Ã»ÓÐÈË½ÐÕâ¸öÃû×Ö");
				}
	...
	teleport_area:
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "ÄãÒÆ¶¯µ½( %d, %d )", x, y);
		ch->WarpSet(x, y);
		ch->Stop();
		lua_pushnumber(L, 1);
		return 1;
	...
	ALUA(pc_change_empire)
	{
		...
		{
			ch->ChatPacket (CHAT_TYPE_INFO, LC_TEXT ("Nationality cannot be changed while in a team."));
			lua_pushnumber(L, 4);
			return 0;
		}
	...
	ALUA (pc_change_name_sx)
	{
		...
		// const char * szNameParantezli = lua_tostring(L,1);

		char szxd[1024];
		snprintf (szxd, sizeof (szxd), "¡ï¡ï¡ï%s¡ï¡ï¡ï", szName);

		char szQuery[1024];
		snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*) FROM player%s WHERE name='%s'", get_table_postfix(), szxd);
		auto pmsg(DBManager::instance().DirectQuery(szQuery));
		...
		/* change_name_log */
		// LogManager::instance().ChangeNameLog (pid, ch->GetName(), szxd, ch->GetDesc()->GetHostName());

		snprintf(szQuery, sizeof(szQuery), "UPDATE player%s SET name='%s' WHERE id=%u", get_table_postfix(), szxd, pid);
		auto msg = DBManager::instance().DirectQuery(szQuery);
	...
	ALUA (pc_change_name_kx)
	{
		...
		char szxd_kx[1024];
		snprintf (szxd_kx, sizeof (szxd_kx), "¡î¡î¡î%s¡î¡î¡î", szName);

		char szQuery[1024];
		snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*) FROM player%s WHERE name='%s'", get_table_postfix(),szxd_kx);
		auto pmsg(DBManager::instance().DirectQuery(szQuery));
		...
		/* change_name_log */
		// LogManager::instance().ChangeNameLog (pid, ch->GetName(), szxd_kx, ch->GetDesc()->GetHostName());

		snprintf(szQuery, sizeof(szQuery), "UPDATE player%s SET name='%s' WHERE id=%u", get_table_postfix(), szxd_kx, pid);
		auto msg = DBManager::instance().DirectQuery(szQuery);

		ch->SetNewName (szxd_kx);
		lua_pushnumber (L, 4);
		return 1;
	}

	////////¹ÜÀíÔ±¸ÄÃû¿¨//////////
	ALUA (pc_change_name_gm)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		char szxd2[1024];
		snprintf (szxd2, sizeof (szxd2), "¡¾%s¡¿", szName);

		char szQuery[1024];
		snprintf(szQuery, sizeof(szQuery), "SELECT COUNT(*) FROM player%s WHERE name='%s'", get_table_postfix(), szxd2);
		auto pmsg(DBManager::instance().DirectQuery(szQuery));
		...
		/* change_name_log */
		// LogManager::instance().ChangeNameLog (pid, ch->GetName(), szxd2, ch->GetDesc()->GetHostName());

		snprintf(szQuery, sizeof(szQuery), "UPDATE player%s SET name='%s' WHERE id=%u", get_table_postfix(), szxd2, pid);
		auto msg = DBManager::instance().DirectQuery(szQuery);
	...
	ALUA(pc_charge_cash)
	{
		...
		int64_t amount = lua_isnumber(L, 1) ? (int64_t)lua_tonumber(L, 1) : 0;
		...
		packet.dwAmount = (int64_t)amount;
	...
	ALUA (pc_get_cash)
	{
		...
		std::unique_ptr<SQLMsg> pMsg (DBManager::instance().DirectQuery ("SELECT cash FROM %s.account WHERE id = '%d'", gB_AccountSQL.c_str(), ch->GetDesc()->GetAccountTable().id));
...
#define PC_MI0L_ARG3	7*2		// (type, value)*7
	...
	ALUA(pc_get_premium_vip)
	{
		LPCHARACTER pChar = CQuestManager::instance().GetCurrentCharacterPtr();

		if (NULL != pChar)
		{
			bool m_isPremium = pChar->IsPremium();

			lua_pushboolean(L, m_isPremium);
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "getcostume_hair",	pc_get_costume_hair},
			{ "get_costume_hair",	pc_get_costume_hair},
			{ "getcostume_body",	pc_get_costume_body},
			...
			{ "change_name_sx",			pc_change_name_sx },
			{ "change_name_kx",			pc_change_name_kx },
			{ "change_name_gm",			pc_change_name_gm },
			{ "is_dead",				pc_is_dead	},
