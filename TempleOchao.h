...
namespace TempleOchao
{
	class CMgr : public singleton<CMgr>
	{
		public:
			...
			void	SetGuardianVID (DWORD dwGuardianVID)
			{
				dw_GuardianVID = dwGuardianVID;
			}
			DWORD	GetGuardianVID()
			{
				return dw_GuardianVID;
			}
			void	SetPortalVID (DWORD dwPortalVID)
			{
				dw_PortalVID = dwPortalVID;
			}
			DWORD	GetPortalVID()
			{
				return dw_PortalVID;
