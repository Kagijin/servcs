...
struct FPartyDropDiceRoll
{
	...
	void Process(const LPCHARACTER mobVictim, PIXEL_POSITION pos = {0,0,0})
#else
...
#endif
	{
		if ((!mobVictim || (mobVictim->GetMobRank() >= MOB_RANK_BOSS && mobVictim->GetMobRank() <= MOB_RANK_KING)) && m_itemOwner->GetParty() && m_itemOwner->GetParty()->GetNearMemberCount() > 1)
		{
			...
			if (m_itemOwner)
			{
				...
				if (m_itemOwner->IsAutoPickUp())
				{
					autoPickup = m_itemOwner->DropItemToInventory(m_itemDrop);
				}
				if (!autoPickup)
				{
					m_itemDrop->AddToGround(m_itemOwner->GetMapIndex(), pos);
					m_itemDrop->SetOwnership(m_itemOwner);
					m_itemDrop->StartDestroyEvent();
				}
#else
				if (!m_itemOwner->DropItemToInventory(m_itemDrop))
				{
					m_itemDrop->AddToGround(m_itemOwner->GetMapIndex(), pos);
					m_itemDrop->SetOwnership(m_itemOwner);
					m_itemDrop->StartDestroyEvent();
