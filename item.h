...
public:
	...
	LPCHARACTER GetOwner() { 
		return (m_pOwner != nullptr) ? m_pOwner : nullptr; 
	}
	...
	void		StartDestroyEvent(int iSec = 60);//地面的物品消失时间
	...
	void		SetOwnership(LPCHARACTER ch, int iSec = 10);//掉落的物品归属权显示时间
...
public:
	void	AddSelectedRareAttributes(const std::vector<BYTE> &vec_attr);
	void	AddSelectedAttributes(const std::vector<BYTE> &vec_attr);
#endif
