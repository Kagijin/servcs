...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkChr->GetMapIndex() == 195)
	{
		pkChr->ChatPacket (CHAT_TYPE_INFO, LC_TEXT ("ALIGNMENT_MAC195_USE_PVP"));//¸ÃµØÍ¼ÒÑÏÞÖÆ×é¶Ó¹¦ÄÜ
		return;
	}
#endif
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	// if (pkChr->IsBotCharacter() || pkVictim->IsBotCharacter())
		// return false; //¸ÄÎª true ¹ÖÎï¿ÉÒÔ¹¥»÷
	if (pkChr->IsBotCharacter())// »úÆ÷ÈË½ÇÉ«µÄ¹¥»÷¿ØÖÆ true or false
	{
		if (pkVictim->IsStone())// »úÆ÷ÈË¿ÉÒÔ¹¥»÷ÔÉÊ¯ true or false
			return true;

		if (pkVictim->IsBotCharacter() && pkChr->GetEmpire() != pkVictim->GetEmpire())// »úÆ÷ÈË¿ÉÒÔ¹¥»÷µÐ·½µÛ¹úµÄ»úÆ÷ÈË
			return false;

		if (pkVictim->IsPC() && !pkVictim->IsBotCharacter() && pkChr->GetEmpire() != pkVictim->GetEmpire())// Íæ¼Ò¿ÉÒÔ¹¥»÷»úÆ÷ÈË
			return true;

		if (pkVictim->IsMonster())// »úÆ÷ÈË¿ÉÒÔ¹¥»÷ÆÕÍ¨¹ÖÎïtrue or false
			return true;
	}
#endif
