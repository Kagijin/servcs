...
namespace fishing
{
	enum
	{
		...
		FISH_BONE_VNUM = 27799,//戴므
		SHELLFISH_VNUM = 27987,//勵瀧곱
		EARTHWORM_VNUM = 27801,//嗔贄
		// WATER_STONE_VNUM_BEGIN = 28030,//샷沂柯+0
		// WATER_STONE_VNUM_END = 28043,//뒨珙柯+0
	...
	int DetermineFish (LPCHARACTER ch)
	{
		...
		#ifdef ENABLE_FISH_DROP_ITEM_80008__
		DWORD vnum = fish_info[fish_idx].vnum;
		if (vnum == 50008 || vnum == 50009 || vnum == 80008 || vnum == 70049)
		{
			return 0;
		}
		#else
		DWORD vnum = fish_info[fish_idx].vnum;
		if (vnum == 50008 || vnum == 50009)
		{
			return 0;
		}
		#endif
			...
			switch (ret)
			{
				...
				case 0:
					if (item_vnum)
					{
						...
						ch->UpdateExtBattlePassMissionProgress(FISH_FISHING, 1, p.info);
#endif
#ifdef ENABLE_PLAYER_STATS_SYSTEM
						ch->PointChange(POINT_FISHING, 1);
#endif
