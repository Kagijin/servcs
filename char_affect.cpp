...
int CHARACTER::ProcessAffect()
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		// if (pkAff->lSPCost > 0 && pkAff->dwType != AFFECT_AUTO_PICK_UP)
		if (pkAff->lSPCost > 0)
		{
...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube)
{
	...
	{
		ChatPacket (CHAT_TYPE_INFO, LC_TEXT ("款康磊 力力肺 盲泼捞 陛瘤 登菌嚼聪促."));
	}
	// END_OF_CHAT_BLOCK
	...
	case SKILL_HOSIN:		//结晶
	case SKILL_REFLECT:		//
	case SKILL_GICHEON:		//凝神
	case SKILL_JEONGEOP:	//
	case SKILL_KWAESOK:		//飘仙
	case SKILL_JEUNGRYEOK:	//神曲
	...
	{
		// if (!pkAff)  // NULL 该功能检查会导致PVP时倒计时不能正常删除移动速度
		// {
...
bool CHARACTER::RemoveAffect(CAffect* pkAff)
{
	...
	// ComputePoints();//仅仅测试刷新点，如果不正常再恢复
	CAffect::Release(pkAff);
	return true;
}
...
void CHARACTER::RemoveGoodAffect()
{
	...
	RemoveAffect(SKILL_MUYEONG);//@Lightwork#35 可能是黑魔灵魂 火球
