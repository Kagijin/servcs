...
namespace quest
{
		...
		int kingdoms_config(lua_State* L)
		{
			...
			lua_pushboolean(L, CMgrKingdomsWar::instance().Config(lua_tonumber(L, 1), lua_tonumber(L, 2), lua_tonumber(L, 3), lua_tonumber(L, 4), lua_tonumber(L, 5), lua_tonumber(L, 6)));
			return 1;
		}
