...
void ITEM_MANAGER::Update()
{
	...
	while (it != m_set_pkItemForDelayedSave.end())
	{
		...
		// IMPROVED CHECK: Checks if the item is null OR if it no longer has an owner.
		if (!item || !item->GetOwner())
		{
		...
		// If the item has an owner, this check is now safe.
		if (IS_SET(item->GetFlag(), ITEM_FLAG_SLOW_QUERY))
			continue;
