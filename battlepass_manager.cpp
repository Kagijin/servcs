...
void CBattlePassManager::BattlePassRequestReward(LPCHARACTER pkChar, BYTE bBattlePassType)
{
	...
	if (bBattlePassType == 1)
	{
		...
		for (int i = 0; i < missionInfo.size(); i++) {
			if (!pkChar->IsExtBattlePassCompletedMission(bBattlePassType, missionInfo[i].bMissionIndex, missionInfo[i].bMissionType)) {
				pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Unfinished ordinary task pass"));
				return;
			}
		}
	}
	if (bBattlePassType == 2)
	{
		...
		for (int i = 0; i < missionInfo.size(); i++) {
			if (!pkChar->IsExtBattlePassCompletedMission(bBattlePassType, missionInfo[i].bMissionIndex, missionInfo[i].bMissionType)) {
				pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Unfinished Advanced Task Pass"));
				return;
			}
		}
	}
	if (bBattlePassType == 3)
	{
		...
		for (int i = 0; i < missionInfo.size(); i++) {
			if (!pkChar->IsExtBattlePassCompletedMission(bBattlePassType, missionInfo[i].bMissionIndex, missionInfo[i].bMissionType)) {
				pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Unfinished Activity Pass"));
				return;
			}
		}
	}

	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT battlepass_completed FROM %s.battlepass_playerindex WHERE player_id = %d and battlepass_type = %d and battlepass_id = %d", gB_PlayerSQL.c_str(), pkChar->GetPlayerID(), bBattlePassType, bBattlePassID));
	if (pMsg->uiSQLErrno)
		return;
	...
	if (bHasRecived != 0) {
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("You have received the task pass reward"));
		return;
	}
	
	DBManager::instance().DirectQuery("UPDATE %s.battlepass_playerindex SET battlepass_completed = 1, end_time = NOW() WHERE player_id = %d and battlepass_type = %d and battlepass_id = %d",gB_PlayerSQL.c_str(), pkChar->GetPlayerID(), bBattlePassType, bBattlePassID);
