...
void MessengerManager::Login(MessengerManager::keyA account)
{
	...
	DBManager::instance().FuncQuery(std::bind1st(std::mem_fun(&MessengerManager::LoadTeamList), this),"SELECT '%s', mName FROM common2.gmlist WHERE mName != '%s' AND mMessengerShow = 1", account.c_str(), account.c_str());
#else
	DBManager::instance().FuncQuery(std::bind1st(std::mem_fun(&MessengerManager::LoadTeamList), this),"SELECT '%s', mName FROM common.gmlist WHERE mName != '%s' AND mMessengerShow = 1", account.c_str(), account.c_str());
#endif
#endif
