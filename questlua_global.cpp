...
namespace quest
{
		...
		for (int type = PRIV_NONE + 1; type < MAX_PRIV_NUM; ++type)
		{
			...
			if (pkPrivEmpireData && pkPrivEmpireData->m_value)
			{
				...
				os << LC_TEXT(c_apszPrivNames[type]) << " : " <<
					pkPrivEmpireData->m_value << "% 加成" << " (有效时间:" <<
					((pkPrivEmpireData->m_end_time_sec - get_global_time()) / 3600.0f) << " 小时)" << endl;
				found = true;
			}
		}
		...
		for (int type = PRIV_NONE + 1; type < MAX_PRIV_NUM; ++type)
		{
			...
			if (pPrivGuildData && pPrivGuildData->value)
			{
				...
				os << LC_TEXT(c_apszPrivNames[type]) << " : " << pPrivGuildData->value << "%"
					<< " (" << ((pPrivGuildData->end_time_sec - get_global_time()) / 3600.0f) << " 小时)" << endl;
				found = true;
			}
		}
	...
	{
		if (!lua_isstring(L, 1)) {
			sys_err("_get_guildid_byname() - 无效的值");
			lua_pushnumber(L, 0);
			return 1;
		}
	...
	ALUA(_warp_all_to_village)
	{
		...
		SendNoticeMap (LC_TEXT ("泪矫饶 葛滴 付阑肺 捞悼邓聪促."), iMapIndex, false);
		...
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet()
					...
					ch->DeadNoReward(); //死亡无奖励 @fixme188 from Dead()
			}
		}
	};
...
#define NAN (INFINITY-INFINITY)
	...
	ALUA (_time_str)
	{
		...
		if (!lua_isnumber (L, 1))
		{
			snprintf (timeStr, sizeof (timeStr), "0小时 0分钟 0秒");
		}
		else
		{
			...
			else
			{
				...
				if (day > 0)
				{
					snprintf (timeStr, sizeof (timeStr), "%d天 %02d小时 %02d分钟 %02d秒", day, hour, minute, second);
				}
				else
				{
					snprintf (timeStr, sizeof (timeStr), "%02d小时 %02d分钟 %02d秒", hour, minute, second);
				}
			}
		}
	...
	ALUA (_kill_vid0)
	{
		LPCHARACTER npc = CHARACTER_MANAGER::instance().Find (lua_tonumber (L, 1));
		if (npc)
		{
			npc->Dead();
		}
