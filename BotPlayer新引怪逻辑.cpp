...
struct FuncFindMultiVictim
{
	...
	void operator()(LPENTITY ent)
	{
		...
		if (!pkChr || pkChr == m_me || pkChr->IsDead()) return;

		// Atacar apenas Monstros e Pedras (Metins)
...
EVENTFUNC(bot_character_event)
{
	...
	FuncFindVictim f(botCharacter, 3000);//发现目标 引怪距离设置
	...
	if (victim)
	{
		...
		if (SECTREE_MANAGER::instance().IsMovablePosition(victim->GetMapIndex(), victim->GetX(), victim->GetY()))
		{
				...
				return PASSES_PER_SEC(0.15f); // 接近目标时保持冷静的等待
			}
		}
		...
		if (distance <= attackRange && SECTREE_MANAGER::instance().IsMovablePosition(victim->GetMapIndex(), victim->GetX(), victim->GetY()))
		{
			...
			// Attack() 函数发送了数据包
		// L?GICA DE ATAQUE EM ?REA
		if (distance <= attackRange)
		{
			// Vira o bot para o alvo principal
			float dx = victim->GetX() - botCharacter->GetX();
			float dy = victim->GetY() - botCharacter->GetY();
			botCharacter->SetRotation(GetDegreeFromPosition(dx, dy));

			// botCharacter->AggregateMonster(); // REMOVIDO

			// 1. Define quantos monstros puxar agora (entre 2 e 5)
			int iPullLimit = number(2, 5);
			int iCountPulled = 0;

			// 2. Procura monstros num raio de 3000 (alcance de aggro)
			...
			// 3. Se houver monstros, embaralha a lista para ser aleatório e puxa a quantidade definida
			if (!aggroCollector.m_targets.empty())
			{
				// Necessário incluir <algorithm> e <random> se der erro, mas seu arquivo já tem.
				...
				for (auto target : aggroCollector.m_targets)
				{
					...
					// Verifica se é monstro, está vivo e (opcional) se já n?o está atacando o bot
					if (target && !target->IsDead() && target->IsMonster())
					{
						// Se o monstro já tiver outro alvo, você pode decidir se rouba o aggro ou n?o.
						// Aqui for?amos ele a atacar o bot:
			...
			// 1. Encontrar TODOS os alvos próximos (Splash Damage)
			// Raio de 300 unidades (ajuste conforme necessário para parecer com o Bot de Memória)
			FuncFindMultiVictim splashCollector(botCharacter, 300);
			if (botCharacter->GetSectree())
			{
			...
			// 2. Iterar sobre todos os alvos encontrados
			for (auto target : splashCollector.m_targets)
			{
				// Verifica se o alvo é atacável (usando fun??o do battle.cpp)
				if (battle_is_attackable(botCharacter, target))
				{
					// For?a o ataque direto
					// Nota: battle_melee_attack calcula o dano real baseado nos stats
					...
					if (iRet != BATTLE_NONE)
					{
						...
						// --- TRUQUE VISUAL ---
						// Envia o efeito de "Explos?o" ou "Crítico" manualmente para simular o bot de memória
						// SE_PENETRATE = Efeito de perfura??o (Brilho roxo/amarelo de impacto)
						// SE_CRITICAL = Efeito de crítico (Explos?o vertical)
						target->EffectPacket(SE_PENETRATE);
						target->EffectPacket(SE_CRITICAL);
					}
				}
			}
			...
			if (attackedAny)
			{
			    // Envia anima??o de ataque do bot (apenas visual, o dano já foi feito acima)
			    botCharacter->SendMovePacket(FUNC_COMBO, MOTION_COMBO_ATTACK_1,
									       botCharacter->GetX(), botCharacter->GetY(),
									       0, get_dword_time());

			    // Retorna velocidade de ataque rápida (igual memory bot)
	...
	{
		// Se n?o tiver monstros, desce do cavalo ocasionalmente
		if (botCharacter->IsHorseRiding() && !victim && std::rand() % 4 == 0)
			botCharacter->StopRiding();

		// 1. Calcula a dist?ncia atual até o ponto de nascimento (Home)
		long distToHome = DISTANCE_APPROX(botCharacter->GetX() - info->startX, botCharacter->GetY() - info->startY);

		// 2. Se estiver muito longe (ex: mais de 3000 unidades / 30 metros), volta para o local de nascimento
		if (distToHome > 3000)
		{
			// Verifica se o caminho é válido e anda de volta
			if (botCharacter->Goto(info->startX, info->startY))
			{
				botCharacter->SendMovePacket(FUNC_WAIT, 0, 0, 0, 0);
			}
		}
		// 3. Se estiver perto de casa, continua vagando aleatoriamente (código original)
...
// #endif
		...
		botCharacter->SetPoint(POINT_ATTBONUS_MONSTER, 100); // 对怪物强悍100%
