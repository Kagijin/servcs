...
bool	g_bEnableSpeedHackCrash = false;//开启速度破解踢下线
#endif
...
//将返回客户端的内网IP 强行更改为外网IP

char		g_szPublicIP[16] = "0";
char		g_szInternalIP[16] = "0";
#ifdef ENABLE_FORCED_PUBLICIP_IP_SYSTEM
char		g_szPublicIP_CONFIG_IP[16] = SERVER_CONFIG_IP;
#endif
bool		g_bSkillDisable = false;
int			g_iFullUserCount = 800;//多少人在线显示爆满
int			g_iBusyUserCount = 50;//多少人在线显示繁忙
...
string gB_AccountSQL = "account";
string gB_PlayerSQL = "player";
string g_stHostname = "";
...
int SPEEDHACK_LIMIT_COUNT = 60;
int SPEEDHACK_LIMIT_BONUS = 90;
#else
int SPEEDHACK_LIMIT_COUNT = 60;
int SPEEDHACK_LIMIT_BONUS = 90;
#endif
...
int g_server_id = 0;
string g_strWebMallURL = "www.yt2004.com";
...
static bool __LoadConnectConfigFile(const char* configName)
{
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fpOnlyForDB))
	{
		...
		TOKEN ("ACCOUNT_SQL_NAME")
		{
			gB_AccountSQL = value_string;
			fprintf (stdout, "ACCOUNT_SQL_NAME: %s\n", gB_AccountSQL.c_str());
			continue;
		}

		TOKEN ("PLAYER_SQL_NAME")
		{
			gB_PlayerSQL = value_string;
			fprintf (stdout, "PLAYER_SQL_NAME: %s\n", gB_PlayerSQL.c_str());
			continue;
		}
		TOKEN("channel")
		{
...
#ifdef ENABLE_FORCED_PUBLICIP_IP_SYSTEM
		TOKEN("SERVER_CONFIG_IP")
		{
			strlcpy(g_szPublicIP_CONFIG_IP, value_string, sizeof(g_szPublicIP_CONFIG_IP));
		}
#endif
		TOKEN("view_range")
		{
