	...
	case POINT_LEVEL:
		...
		if (GetJob() == JOB_WOLFMAN)
		{
			if ((5 <= val) && (GetSkillGroup() != 1))
			{
				...
				// WolfmanSkillSet();//¼ÓÈëÃÅÅÉÀÇÈËÖ±½ÓG1¼¼ÄÜ¸¨ÖúM1
			}
		}
#endif
		...
		if (amount)
		{
			...
			if (biodurum == 0 && GetLevel() >= 30)
			{
				...
				int biostate = GetQuestFlag("bio.ruhtasi");
				biodurum = GetQuestFlag("bio.durum");
				ChatPacket(CHAT_TYPE_COMMAND, "biyolog_update %d %d %d %d %d", biodurum, biostate, bioverilen, BiyologSistemi[biodurum][1], biokalan);
			}
			else
			{
				int bioverilen = GetQuestFlag("bio.verilen");
				int biokalan = GetQuestFlag("bio.kalan")-get_global_time();
				int biostate = GetQuestFlag("bio.ruhtasi");
				biodurum = GetQuestFlag("bio.durum");
				ChatPacket(CHAT_TYPE_COMMAND, "biyolog_update %d %d %d %d %d", biodurum, biostate, bioverilen, BiyologSistemi[biodurum][1], biokalan);
			}
...
bool CHARACTER::Follow(LPCHARACTER pkChr, float fMinDistance)
{
	...
	if (GetMobRank() >= 4)
	{
		if (2000 < DISTANCE_APPROX(m_pkMobInst->m_posCreate.x - GetX(), m_pkMobInst->m_posCreate.y - GetY()))
			if (Return(true))
				return true;
	}
#endif
...
EVENTFUNC(check_speedhack_event)
{
	...
	{
		if (g_bEnableSpeedHackCrash)//¿ªÆôËÙ¶ÈÆÆ½âÌßÏÂÏß
		{
