...
)
{
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		if (loader.GetTokenString("type", &stType))
		{
			...
			else if (stType == "const") {//开出所有物品 多出的物品扔到地上
				type = CSpecialItemGroup::CR_CONST;
			}
			else if (stType == "stone") {//newpct 多出的物品扔到地上,唯一不同的是支持一次打开800个宝箱
				type = CSpecialItemGroup::CR_STONE;
			}
			else if (stType == "newpct") {//按概率开出多个物品 类似于pct 类型
				type = CSpecialItemGroup::CR_NEW_PCT;
			}
			else if (stType == "norm") {//与正常宝箱开启相同
		...
		if (strType == "kill")
		{
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector (buf, &pTok))
				{
					...
					if (!GetValidVnum(name.c_str(), dwVnum) && !GetVnumByOriginalName(name.c_str(), dwVnum))
					{
						sys_err("ReadMonsterDropItemGroup : there is no item %s : node %s : vnum %d", name.c_str(), stName.c_str(), dwVnum);
						return false;
					}
