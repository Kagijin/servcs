...
void CSafebox::ChangeSize(int iSize)
{
	...
	if (pkOldGrid) {
		...
		delete pkOldGrid; // @lightwork memory leak fix
	}
	else {
		m_pkGrid = M2_NEW CGrid(5, m_iSize);
	}
...
LPITEM CSafebox::GetItem(CELL_UINT bCell)
{
	if (bCell >= 5 * m_iSize)
	{
...
bool CSafebox::MoveItem(CELL_UINT bCell, CELL_UINT bDestCell, MAX_COUNT count)
#else
bool CSafebox::MoveItem(CELL_UINT bCell, CELL_UINT bDestCell, BYTE count)
#endif
{
	LPITEM item;
