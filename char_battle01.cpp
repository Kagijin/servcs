...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (isDuel == false)
	{
		...
		else
		{
			...
			// {
				// m_pkDeadEvent = event_create(dead_event, pEventInfo, 0);//修改2025-01-22
...
#endif
{
	...
	if ((GetRaceNum() == 8020) || (GetRaceNum() == 8021) || (GetRaceNum() == 8022))
	{
		SendDamagePacket (pAttacker, 0, DAMAGE_BLOCK);
		if (pAttacker->GetFakeStoneCounter() >= 50)
		{
		...
		{
			pAttacker->ChatPacket (CHAT_TYPE_INFO, "[%s] 这是假陨石继续攻击 %d 次后将被踢出游戏!", pAttacker->GetName(), 50 - pAttacker->GetFakeStoneCounter());
...
#ifdef ENABLE_BOT_CONTROL
				if (m_me->IsBotControl() && pkVictim && pkVictim->IsStone())
				{
					sys_log(0, "[BOT CONTROL] %s tried to hit stone %s", m_me->GetName(), pkVictim->GetName());
					return;
				}
#endif
...
bool CHARACTER::GetAttackHack()
{
	...
	if (m_mapAttackCount.size() > 50 )//同时击中数量大于5个目标踢下线
	{
		// HackLogAdd();	//踢下线
		return true;
	}
...
void CHARACTER::CalculateAttackCount()
{
	...
	BYTE attCount = 0;
	BYTE extraTolerance = 50;	// 调整了攻击次数 减少对玩家的惩罚
	switch (GetJob())
	{
		case JOB_ASSASSIN:
		{
	...
	// 限制额外添加了 +30 攻击给予玩家余地并减少不必要的惩罚

	attCount += extraTolerance;
