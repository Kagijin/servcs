...
void CMountActor::SetName()
{
	...
	else
	{
		if (0 != m_pkOwner && 0 != m_pkOwner->GetName())
			snprintf(buf, sizeof(buf), "%s的%s", m_pkOwner->GetName(), m_pkChar->GetMobTable().szLocaleName);
		else
...
{
	// if (m_pkOwner->GetMapIndex() == 26 || m_pkOwner->GetMapIndex() == 110 || m_pkOwner->GetMapIndex() == 202)
	// {
		// m_pkOwner->ChatPacket (CHAT_TYPE_INFO, LC_TEXT ("Mounts cannot be used on pvp maps"));
		// return false;//try
	// }
	...
	//上马删除火球技能
#ifdef ENABLE_SKILL_MUYEONG_FIX
...
{
	//进入战场不能召唤坐骑
	if (m_pkOwner->GetMapIndex() == 195 || m_pkOwner->GetMapIndex() == 113 || m_pkOwner->GetMapIndex() == 202 || m_pkOwner->GetMapIndex() == 26 || m_pkOwner->GetMapIndex() == 110) // // In ox you can't summon your pet.
		return 0;
